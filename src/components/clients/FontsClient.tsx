...
import { getAllFonts, type FontItem } from '@/lib/datasetLoader';
import { 
  Type, 
  Search, 
  Copy, 
  ...
  const filteredFonts = useMemo(() => {
    return allFonts.filter(font => {
      const matchesSearch = 
        ...
        font.designers.some(d => d.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  return (
    <div className="h-full flex flex-col p-3 overflow-hidden gap-3 font-mono">
      ...
      <section className="bg-[var(--bg-panel)] border border-[var(--border-dev)] rounded-xl p-4 shrink-0 shadow-sm transition-colors flex flex-col gap-3">
        ...
        <div className="bg-[var(--bg-codebox)] border border-[var(--border-dev)] rounded-lg p-4 overflow-x-auto min-h-[90px] flex items-center transition-colors">
          <input
            type="text"
            value={sampleText}
      ...
      <div className="bg-[var(--bg-panel)] border border-[var(--border-dev)] rounded-lg px-3 py-2 flex flex-wrap items-center justify-between gap-3 shrink-0 shadow-sm transition-colors">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-[10px] text-[var(--text-muted)]">LICENSE:</span>
          <select
